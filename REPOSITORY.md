...
| `docs/preregistrations/` | Hashed and committed before the run they govern. `*.draft.*` files are generated by `scripts/draft_preregistration.py`, carry `<<FILL>>` placeholders, and are refused by the report gate until committed. |
